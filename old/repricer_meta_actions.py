...
class MetaActionExecutor:
    ...
    def _initial_index(self) -> int:
        ...
        price = self.rng.uniform(lower, float(self.price_grid[-1]))
    ...
    def next_price_index(self, competitor_min_idx: int) -> int:
        ...
        if self.action.base_rule == "undercut":
        ...
        if self.action.reset_when_below_cost:
            if self.price_grid[next_idx] <= self.marginal_cost + 1e-12:
                next_idx = grid_last

        if self.action.raise_if_below_min:
            if self.current_index < competitor_min_idx:
                next_idx = min(next_idx + 1, grid_last)
...
def create_action_executor(
    ...
    return MetaActionExecutor(action, price_grid, marginal_cost, rng)
