...
class NPlayerLogitDemandEnv:
    ...
    def __init__(
        ...
        max_price: Optional[float] = None, # new manual max price option, can delete later
    ) -> None:
        """
        ...
        # Baseline per-seller profits at the symmetric Nash and monopoly prices.
        self.per_seller_profit_nash = float(
            self._per_seller_profit_at_price(self.p_nash, mc=base_mc)
        )
        self.per_seller_profit_monopoly = float(
            self._per_seller_profit_at_price(self.p_monopoly, mc=base_mc)
        )

        # Added manually max_price option for more flexible grid setting. Can delete it later.
        if max_price is not None:
            self.prices = np.linspace(
                self.p_nash, max_price, grid_size
            )
        else:
    ...
    # ------------------------------------------------------------------
    def _per_seller_profit_at_price(self, price: float, mc: float) -> float:
        """Return per-seller profit when all firms charge the same price."""
        return self._profit_given_others_price_logit(price, price, mc=mc)
    ...
    def run_episode(
        ...
        # Build action executors for each seller. Notice since we do it every episode, the initial price always resets to a random one, unless the seller has initial_price_indices set and is not static.
        executors: List = []
