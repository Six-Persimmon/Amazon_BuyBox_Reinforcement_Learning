#!/usr/bin/env python3
"""
Progress report for exp08 async-Poisson runs (K_VALUES = lambda_K values).

Usage:
  python exp08_progress_report.py --rounds 100
  python exp08_progress_report.py --root /path/to/data/results_exp08 --rounds 100
"""
...
SCAN_DIR_RE = re.compile(r"^scan_async_(3strats|4strats)_mu(.+)_lam(.+)$")


def _default_root() -> Path:
    bigdata = Path("~/bigdata/pricing_marl/data/results_exp08").expanduser()
    if bigdata.exists():
        return bigdata
    return Path(__file__).resolve().parent.parent / "data" / "results_exp08"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Progress report for exp08 async-Poisson runs (K_VALUES = lambda_K values).")
    parser.add_argument("--root", type=str, default=None, help="Path to data/results_exp08")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to exp08_async_poisson.py (to infer N/Mu/K values)",
    )
    ...
    grid_file = Path(args.grid_file).expanduser() if args.grid_file else (
        Path(__file__).resolve().parent.parent / "experiments" / "exp08_async_poisson.py"
    )
    ...
    for label in labels:
        scan_dirs_all = sorted(root.glob(f"scan_async_{label}_*"))
        ...
        if n_values is not None and mu_values is not None and k_values is not None:
            ...
            for mu_val in mu_values:
                for k_val in k_values:
                    scan = root / f"scan_async_{label}_mu{mu_val}_lam{k_val}"
