#!/usr/bin/env python3
"""
Progress report for exp06 weighted-exploration runs (4strats only).

Usage:
  python exp06_progress_report.py --rounds 100
  python exp06_progress_report.py --root /path/to/data/results_exp06 --rounds 100
"""
...
SCAN_DIR_RE = re.compile(r"^scan_wexp_(3strats|4strats)_mu(.+)_k(.+)$")


def _default_root() -> Path:
    bigdata = Path("~/bigdata/pricing_marl/data/results_exp06").expanduser()
    if bigdata.exists():
        return bigdata
    return Path(__file__).resolve().parent.parent / "data" / "results_exp06"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Progress report for exp06 weighted-exploration runs (4strats only).")
    parser.add_argument("--root", type=str, default=None, help="Path to data/results_exp06")
    parser.add_argument("--rounds", type=int, default=30, help="Runs per N/mu/set")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to exp06_weighted_explore.py (to infer N/Mu/K values)",
    )
    ...
    grid_file = Path(args.grid_file).expanduser() if args.grid_file else (
        Path(__file__).resolve().parent.parent / "experiments" / "exp06_weighted_explore.py"
    )
    ...
    labels = [args.experiment_set] if args.experiment_set else ["4strats"]
    ...
    for label in labels:
        scan_dirs_all = sorted(root.glob(f"scan_wexp_{label}_*"))
        ...
        if n_values is not None and mu_values is not None and k_values is not None:
            ...
            for mu_val in mu_values:
                for k_val in k_values:
                    scan = root / f"scan_wexp_{label}_mu{mu_val}_k{k_val}"
